...
generatorName=function generatorName(generator){
switch(generator){
...
var joins=TL.andList(this.joinLeave['join']);
var leaves=TL.andList(this.joinLeave['leave']);
var joinedMessage=this.joinLeave['join'].length?TL([""," joined the tournament"],joins):'';
var leftMessage=this.joinLeave['leave'].length?TL([""," left the tournament"],leaves):'';
var sentence=joinedMessage&&leftMessage?
TL(["","; ",""],joinedMessage,leftMessage):joinedMessage||leftMessage;
var message=TL(["","."],sentence);
...
var type=ChatTournament.generatorName(this.info.generator);
return TL([""," "," tournament"],formatName,type);
};_proto.
...
var generatorName=ChatTournament.generatorName(tournament.generator);
var started=tournament.isStarted?' '+TL(["(started)"]):'';
buf+="<li>";
buf+=eHTML(["<a class=\"ilink\" href=\"","\">","</a>"],toRoomid(tournament.room),tournament.room);
...
var type=ChatTournament.generatorName(args[3]);
var _tourName=this.tournamentName();
var _buf=eHTML(["<div class=\"tournament-message-create\">","</div>"],TL([""," created."],_tourName));
if(!this.tryAdd("|html|"+_buf)){
var hiddenBuf=eHTML(["<div class=\"tournament-message-create\">","</div>"],TL([""," created (and hidden)."],_tourName));
this.room.add("|html|"+hiddenBuf);
}
if(notify){
this.room.notify({
title:TL(["Tournament created"]),
body:TL.label(TL(["Room"]),this.room.title)+"\n"+TL.label(TL(["Format"]),_formatName)+"\n"+TL.label(TL('Type','kind'),type),
...
case'replace':{
this.tryAdd('||'+TL([""," has joined the tournament, replacing ","."],args[3],args[4]));
break;
}
...
var participants=data[0]?' '+TL(["("," players)"],data[0]):"";
this.room.add("|html|<div class=\"tournament-message-start\">"+TL(["The tournament has started!"])+participants+"</div>");
break;

case'disqualify':
var dqName=BattleLog.escapeHTML(data[0]);
this.tryAdd("|html|<div class=\"tournament-message-disqualify\">"+TL([""," has been disqualified from the tournament."],dqName)+"</div>");
break;
...
var autodqMessage=minutes===1?
TL(["The tournament's automatic disqualify timer has been set to "," minute."],minutes):
...
PS.alert(TL(["Please respond to the tournament within "," seconds or you may be automatically disqualified."],seconds));
if(notify){
this.room.notify({
title:TL(["Tournament automatic disqualification warning"]),
body:TL.label(TL(["Room"]),this.room.title)+"\n"+TL.label(TL(["Time"]),TL([""," sec"],seconds)),
...
var autostartMessage=_minutes===1?
TL(["The tournament will automatically start in "," minute."],_minutes):
TL(["The tournament will automatically start in "," minutes."],_minutes);
...
var bsP1=BattleLog.escapeHTML(data[0]);
var bsP2=BattleLog.escapeHTML(data[1]);
this.tryAdd("|uhtml|tournament-"+roomid+"|<div class=\"tournament-message-battlestart\"><a href=\""+roomid+"\" class=\"ilink\">"+TL(["Tournament battle between "," and "," started."],bsP1,bsP2)+"</a></div>");
break;
}

case'battleend':{
var beP1=BattleLog.escapeHTML(data[0]);
var beP2=BattleLog.escapeHTML(data[1]);
var score=BattleLog.escapeHTML(data[3].split(',').join(' - '));
var sentence=data[2]==='win'?TL([""," has won the match "," against ",""],beP1,score,beP2):
data[2]==='loss'?TL([""," has lost the match "," against ",""],beP1,score,beP2):
TL([""," has drawn the match "," against ",""],beP1,score,beP2);
...
var winners=TL.andList(endData.results[0]);
var tourName=this.tournamentName();
this.room.add(eHTML(["|html|<div class=\"tournament-message-end-winner\">","</div>"],TL(["Congratulations to "," for winning the ","!"],winners,tourName)));
if(endData.results[1]){
var runnersUp=TL.andList(endData.results[1]);
var runnerUpMessage=endData.results[1].length>1?
TL.label(TL(["Runners-up"]),runnersUp):TL.label(TL(["Runner-up"]),runnersUp);
this.tryAdd(eHTML(["|html|<div class=\"tournament-message-end-runnerup\">","</div>"],runnerUpMessage));
}
...
renderTournamentTools=function renderTournamentTools(){var _info$challenges2,_info$challengeBys,_info$challengeBys2,_info$challenges3,_info$challenges4,_info$challengeBys3,_info$challenges5;
...
preact.h("p",null,
preact.h("button",{"data-cmd":"/tournament join","class":"button"},preact.h("strong",null,TL(["[Join]"])))," ",
preact.h("button",{onClick:this.toggleBoxVisibility,"class":"button"},TL(["[Close]"]))
)
);
}
...
var challengeBys=(_info$challengeBys2=info.challengeBys)!=null&&_info$challengeBys2.length?TL.orList(info.challengeBys):'';
return preact.h("div",{"class":"tournament-tools"},
preact.h(TeamForm,{
...
preact.h("button",{name:"validate","class":"button"},preact.h("i",{"class":"fa fa-check","aria-hidden":true})," ",TL(["[Validate]"])),
" ",
!!(!info.isStarted&&info.isJoined)&&
preact.h("button",{"data-cmd":"/tournament leave","class":"button"},TL(["[Leave]"])),

info.isStarted&&noMatches&&
preact.h("div",{"class":"tournament-nomatches"},TL(["Waiting for battles to become available..."])),

!!((_info$challenges4=info.challenges)!=null&&_info$challenges4.length)&&preact.h("div",{"class":"tournament-challenge"},
preact.h("div",{"class":"tournament-challenge-user"},TL(["vs. ",""],info.challenges[tour.selectedChallenge])),
preact.h("button",{type:"submit","class":"button"},preact.h("strong",null,TL(["[Ready!]"]))),
...
!!((_info$challengeBys3=info.challengeBys)!=null&&_info$challengeBys3.length)&&preact.h("div",{"class":"tournament-challengeby"},
(_info$challenges5=info.challenges)!=null&&_info$challenges5.length?
TL(["Or wait for "," to challenge you."],challengeBys):
TL(["Waiting for "," to challenge you."],challengeBys)
),
!!info.challenging&&preact.h("div",{"class":"tournament-challenging"},
preact.h("div",{"class":"tournament-challenging-message"},TL(["Waiting for ","..."],info.challenging)),
preact.h("button",{"data-cmd":"/tournament cancelchallenge","class":"button"},TL(["[Cancel]"]))
),
!!info.challenged&&preact.h("div",{"class":"tournament-challenged"},
preact.h("div",{"class":"tournament-challenged-message"},TL(["vs. ",""],info.challenged)),
preact.h("button",{type:"submit","class":"button"},preact.h("strong",null,TL(["[Ready!]"])))
)
)
);
...
preact.h("span",{"class":"tournament-status"},info.isStarted?TL(["In progress"]):TL(["Signups"])),
tour.tournamentName(),
...
preact.h("i",{"class":"fa fa-arrows-alt","aria-hidden":true})," ",TL(["[Pop-out]"])
)
...
var userCount="<b>"+((users==null?void 0:users.length)||0)+"</b>";
if(users!=null&&users.length){
var userCountText=users.length===1?TL([""," user"],userCount):TL([""," users"],userCount);
div.innerHTML=TL.label(userCountText)+"<br />"+BattleLog.escapeHTML(users.join(", "));
}else{
div.innerHTML=TL([""," users"],userCount);
}
return div;
}
if(!window.d3){
